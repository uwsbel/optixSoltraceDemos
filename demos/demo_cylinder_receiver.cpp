...
std::string loadPtxFromFile(const std::string& kernel_name) {
    ...
    const std::string ptx_file = std::string(SAMPLES_PTX_DIR) + "demo_cylinder_receiver_generated_" + kernel_name + ".cu.ptx";
...
void createSBT(SoltraceState& state, std::vector<GeometryData::Parallelogram>& helistat_list, std::vector<GeometryData::Cylinder_Y> receiver_list)
{
    ...
    // Hitgroup program record
    {
        ...
        for (int i = 0; i < num_receivers; i++) {
            ...
            OPTIX_CHECK(optixSbtRecordPackHeader(
                ...
                &hitgroup_records_list[sbt_idx]));
            hitgroup_records_list[sbt_idx].data.geometry_data.setCylinder_Y(receiver_list[i]);
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        createContext(state);
        createGeometry_cyl_receiver(state, heliostat_list, receiver_list);
